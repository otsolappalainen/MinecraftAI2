...
MODEL_PATH_NEW = "models_new_v3"
LOG_DIR = "tensorboard_logs_new_v3"
...
class FullModelFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space):
        ...
        super(FullModelFeatureExtractor, self).__init__(observation_space, features_dim=512)
        ...
        if ADJUST_NETWORK_ARCHITECTURE:
            # Scalar processing layers with 256 and 128 neurons
            ...
            # Fusion layers with 512 and 256 neurons
            # Inject a dummy vector (e.g., zeros) for image CNN output
            # Here, we define a dummy vector of size 512
            self.register_buffer('dummy_image_features', th.zeros(512, dtype=th.float32))

            self.fusion_layers = nn.Sequential(
                nn.Linear(scalar_output_size + 512, 512),
                nn.ReLU(),
                nn.Linear(512, 256),
                nn.ReLU(),
            )
            final_features_dim = 256
...
def create_simplified_model(env):
    ...
    policy_kwargs = dict(
        ...
        net_arch=[128, 64, 32],  # Decision layers with decreasing neurons
    )
    return DQN(
