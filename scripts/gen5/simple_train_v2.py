...
MODEL_PATH_NEW = "models_new_v2"
LOG_DIR = "tensorboard_logs_new_v2"
...
class FullModelFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space):
        ...
        if ADJUST_NETWORK_ARCHITECTURE:
            # Scalar processing layers with 64 neurons each
            ...
            # Fusion layers with 128, 64, and 32 neurons
            # Inject a dummy vector (e.g., zeros) for image CNN output
            # Here, we define a dummy vector of size 128
            self.register_buffer('dummy_image_features', th.zeros(128, dtype=th.float32))
...
def create_simplified_model(env):
    ...
    policy_kwargs = dict(
        ...
        net_arch=[64, 32, 16],  # Decision layers with decreasing neurons
    )
    return DQN(
