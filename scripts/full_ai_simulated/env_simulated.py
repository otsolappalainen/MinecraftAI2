...
class SimulatedEnvGraphics(gym.Env):
    ...
    """
    def __init__(self, render_mode="none", grid_size=500, cell_size=50, task_size=20, max_episode_length=2000, simulation_speed=60, zoom_factor=30):
    ...
    def _render(self):
        ...
        agent_x = int((self.x + center_offset) * self.cell_size / self.grid_size * self.window_size)
        agent_z = int((self.z + center_offset) * self.cell_size / self.grid_size * self.window_size)

        # Draw agent (triangle arrow)
        agent_direction = np.radians(self.yaw)
        agent_triangle = [
            (agent_x + 10 * np.cos(agent_direction), agent_z + 10 * np.sin(agent_direction)),
            (agent_x + 5 * np.cos(agent_direction + 2.5), agent_z + 5 * np.sin(agent_direction + 2.5)),
            (agent_x + 5 * np.cos(agent_direction - 2.5), agent_z + 5 * np.sin(agent_direction - 2.5)),
        ]
        ...
        text = font.render(f"X: {self.x:.2f}, Z: {self.z:.2f}, Reward: {self.cumulative_reward:.2f}", True, (0, 0, 0))
        text_rect = text.get_rect(center=(agent_x, agent_z - 15))
