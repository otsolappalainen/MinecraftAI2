...
LEARNING_RATE = 3e-6  # PPO learning rate
N_STEPS = 1024
BATCH_SIZE = 128
N_EPOCHS = 10
GAMMA = 0.99
...
class ResNet18FeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=256, debug=False, log_dir=None, get_step_fn=None):
        ...
        target_block_dim = observation_space.spaces["target_block"].shape[0] # 2
        player_state_dim = observation_space.spaces["player_state"].shape[0] # 8
        surrounding_blocks_shape = observation_space.spaces["surrounding_blocks"].shape # 13x13x4
        # total inventory features = hand(5) + target_block(2) + blocks(4) + player_state(8) = 19

        # Load pretrained ResNet18
        # Input to ResNet: We'll upsample 112x112 to 224x224 inside forward()
        resnet = models.resnet18(pretrained=True)

        for name, param in resnet.named_parameters():
            # Freeze everything except:
            # - layer4 (final ResNet block)
            # - fc layer (though we remove it anyway)
            if not name.startswith('layer4'):
                param.requires_grad = False

        print("[INFO] Frozen all ResNet layers except final block (layer4)")

        # Remove the final classification layer, keep up to the avgpool
        modules = list(resnet.children())[:-1]  # remove fc layer
        self.image_cnn = nn.Sequential(*modules)  # This outputs 512-d features

        # Inventory net: 19 -> 512 -> 128
        inv_input_dim = hand_dim + target_block_dim + blocks_dim + player_state_dim
        self.inventory_net = nn.Sequential(
            nn.Linear(inv_input_dim, 128),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(128, 128),
            nn.ReLU(),
        ...
        # Surrounding net: 13x13x4 = 676 -> 512 -> 128
        surround_input_dim = surrounding_blocks_shape[0]*surrounding_blocks_shape[1]*surrounding_blocks_shape[2]
        self.surrounding_fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(surround_input_dim, 512),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(512, 128),
            nn.ReLU()
        ...
        # Final fusion: image(512) + inv(128) + surround(128) = 768 -> 1024 -> 256 = features_dim
    ...
    def freeze_layers(self):
        ...
        if IMAGE_FUSION_FREEZE:
            # No dedicated image_fc layer now, we directly get 512 from resnet
            pass
    ...
    def forward(self, observations):
        ...
        # Upsample image from (3x112x112) to (3x224x224) for ResNet18
        image = F.interpolate(image, size=(224, 224), mode='bilinear', align_corners=False)
        # Extract image features: ResNet outputs [B, 512, 1, 1]
